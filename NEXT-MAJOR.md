...
- ~~**Remove the `--lock` compatibility shim**~~ — **done on `v3`**. It shipped in `3.0.0-beta.4`/`beta.5` so the rollout would not require migrating every v2 call site — pipelines, Dockerfiles and agent prompts that type `mops install --lock update` — at once. Being a hidden, value-ignoring parse-level accommodation, it reverted cleanly: `cli/legacy-lock-flag.ts`, its five `addOption(legacyLockOption())` call sites and `cli/tests/legacy-lock-flag.test.ts` are deleted, and `--lock` is an unknown option again.
