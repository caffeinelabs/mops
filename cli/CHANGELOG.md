...
- `mops watch`'s file globbing now ignores `dist/`, `build/` and `bundle/` as `mops test`/`lint`/`format` already did; those directories were only ignored on the watch command's own path, so `mops watch` reported errors and warnings from build output that other commands skipped.
