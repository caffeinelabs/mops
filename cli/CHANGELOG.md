...
- **Breaking**: `mops build`, `mops check`, `mops check-stable`, `mops check-candid`, `mops test`, `mops bench` and `mops generate candid` no longer silently ignore `mops.lock` when installing dependencies. They now follow the same default lock flow as `mops install`: create or refresh the lock when it is stale, and fail on a tampered one (e.g. locally edited `.mops/` files). If these commands now fail for you, the lockfile was already invalid — a plain `mops install` would have failed the same way; run `mops install --lock update` to regenerate it. `mops sources` (machine-parsed by the dfx packtool) still leaves the lock untouched.
=======
...
>>>>>>> f4559b2e (docs: document the v3 lock model, mops verify, and the commit-everything guidance)
