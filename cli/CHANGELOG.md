...
- **Breaking**: replica tests, `mops bench`, `--check-deploy` and `mops toolchain bin pocket-ic` require an explicit `[toolchain] pocket-ic` pin. Unpinned, they error naming `mops toolchain use pocket-ic 15.0.0`. There is no silent default. Attaching to an already-running server via `MOPS_POCKET_IC_URL` (added in 2.24.0) still works without a pin.
...
- **Batched registry hash fetches.** Verifying freshly downloaded packages against the registry's file hashes — an update call that goes through consensus (~2 s) — now shares one `getFileHashesByPackageIds` request between the packages that ask for it within the same tick, instead of one request per package. On installs without a usable `mops.lock`, that request starts before the downloads do, so the consensus round overlaps them, and it covers the root dependencies plus — when a stale but readable `mops.lock` is present — the transitive packages that lock still names; lock generation then reuses the answers for free. Packages first discovered later in the install walk are batched among themselves as they arrive. A request is chunked at 50 package ids so its reply cannot outgrow the IC's 2 MB message limit, however large a dependency graph gets. Installs that run from a committed lock are unchanged — they never ask the registry for hashes — and neither `mops sources` nor any other command that skips the lock makes the call.
- **`mops update` is single-pass.** All outdated dependencies now download in parallel through the same bounded pool `mops install` uses — sharing one download with any transitive request for the same package, and starting the batched registry hash fetch before the downloads so the consensus round overlaps them — `mops.toml` is rewritten once after the installs instead of once per package (so the dependency graph is re-resolved once, not once per updated package), and the registry is notified of the installs in one batched call. Registry and GitHub update lookups also run concurrently. Per-package failure handling is unchanged: a dependency that fails to update is reported, keeps its old `mops.toml` entry and exits `2`, without blocking the other updates; a failure no retry can fix is no longer retried alongside a sibling's transient one. Each applied update is reported as `Updated <pkg> <old> -> <new>`.
...
- **Breaking**: replica tests, `mops bench`, `--check-deploy`, and `mops toolchain bin pocket-ic` require an explicit `[toolchain] pocket-ic` pin. An unpinned project errors naming `mops toolchain use pocket-ic 15.0.0` — that version is a hint, not a runtime fallback, so it can move without changing anyone's replica.
- **Breaking**: the legacy `pic-ic` PocketIC client is gone, and with it `[toolchain] pocket-ic` pins below `9.0.0` (deprecated in 2.20). Migration: `mops toolchain use pocket-ic 15.0.0`. A `< 9.0.0` pin fails with a message naming that fix rather than an opaque `BinTimeoutError` — which is all `5.x`–`8.x` pins ever produced anyway, since `pic-ic` spoke only the `4.0.0` protocol. There is no upper bound: mops keeps no list of blessed `pocket-ic` versions, same as for `moc`, `wasmtime` and `lintoko`.
