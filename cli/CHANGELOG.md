...
## Next
- `mops --help` now groups commands by task — start a project, manage dependencies, publish, development, toolchain — instead of one flat list in registration order, so the CLI is explorable without the docs site. Entries are `name, alias` with a one-line summary that fits an 80-column terminal; argument signatures and options moved to `mops <command> --help`.
- Command groups say what they are for, not what they are called. `mops toolchain` reads "Install and inspect the tools mops builds with" where it used to list the tools' names in its description, and `mops user`, `mops self`, `mops owner`, `mops maintainer`, `mops migrate` and `mops docs` got the same treatment. `mops deployed` keeps its scoping prose, moved to the argument description, and its one-line summary is now just the hook's purpose. This is the one line a user sees after guessing a group name.
- Every command prints its usage block after a usage error. `mops add` with no argument used to stop at `error: missing required argument 'pkg'`; it now also shows the usage, the accepted `<pkg>` forms, and the available options.
- Every command's `--help` lists its arguments under an `Arguments:` heading, each with a description. Commander renders that section only for arguments registered with a description, so a command that declared its argument in the usage line alone showed the signature and nothing about what it takes; the same went for an argument registered without one. Twenty-eight commands and subcommands now carry the section, from `mops add <pkg>` to `mops migrate new <name> [canister]` and `mops user set <prop> <value>`, where on main only the four `mops toolchain` subcommands had it.
- `mops cache` is a real command group. It was a single command taking a `size|clean|show` choice, so `mops cache --help` printed `Usage: mops cache [options] <sub>` and said nothing about the three, and a bare `mops cache` errored with `missing required argument 'sub'`. The three are now subcommands, listed by both `mops cache` and `mops cache --help`. `mops cache <unknown>` now reports `unknown command '<unknown>'`, where it used to report `command-argument value '<unknown>' is invalid for argument 'sub'. Allowed choices are size, clean, show.` (both exit 1). `--global` is now an option of `mops cache clean`, the only subcommand that reads it, so it must follow `clean`: `mops cache clean --global` as before, where `mops cache --global clean` was also accepted and now fails with `unknown option '--global'`.
- `mops template` accepts its argument as a validated choice, and `--help` lists every available template. `mops template <name>` — the non-interactive form the docs recommend — used to fail with `error: too many arguments for 'template'. Expected 0 arguments but got 1.`, because the command declared no argument and passed nothing to the implementation, so the picker always ran. `--copyright-owner <owner>` is also registered: the implementation has always read it, but no flag was ever declared, so it was reachable only from the `init` wizard's internal call.
- `mops bump` validates its `[part]` argument as `major`, `minor` or `patch`, so a typo fails up front rather than being treated as absent.
- `--verbose` reads `Show more information` on every command that takes it. It was split between that and `Verbose console output`/`Verbose output`, so the same flag on `mops build` and `mops add` was described two different ways. `mops bench --verbose` keeps its own description, which names the pipeline stages it prints.
- Commander's own entries read in mops' sentence case. `-h, --help` was described as `display help for command` and the `help` command as `Show help for a command` in the same list, and `-v, --version` as `output the version number`; the flags now read `Show help` and `Show the version`.
...
- `mops test`, `mops bench`, `mops format`, `mops lint`, `mops sync` and `mops watch` no longer pick up sources from a git worktree or submodule nested inside the project. The ignore list only pruned the *contents* of `.git`, never the checkout beside it — and a linked worktree has `.git` as a file, which no ignore glob can prune — so a `worktree/` directory in the project made `mops test` run the copy's tests (and `mops lint`/`mops format`/`mops sync` treat its sources as the project's). Any directory below the project root that is its own checkout is now skipped, whether `.git` is a file or a directory.
- `mops test`/`lint`/`format`/`sync`/`bench` now ignore `build/` and `bundle/`, and `mops watch` joins them and the rest in ignoring `.dfx/` and `dist/`. Those three directories were previously ignored on the watch command's own path only, so build output left behind by other tooling was skipped when watching but treated as project sources by every other command.
...
- `mops self update` no longer prints `Success` when the update did not take effect. It trusted the package manager's exit code, but `npm add -g` installs into npm's own global prefix — when another `mops` install (bun, Volta, a second Node) shadows that prefix on PATH, the shell kept running the old version after a "successful" update. It now checks the version the `mops` on PATH reports and fails when it still reports the old one, naming the stale binary and where the package manager installs (`npm prefix -g`). When the update lands earlier on PATH than the copy that was running (two nvm Node versions, say), it succeeds and names the leftover copy, with a `hash -r` hint for a shell that still reports the old version. A project-local `ic-mops` (`node_modules/.bin/mops`, as under `npx mops self update`) was never reachable by the global install either; it is now refused up front with `npm i -D ic-mops@<version>` as the fix.
- `mops self update` and `cli.mops.one/install.sh` work on npm 12, which refuses remote tarballs by default (`EALLOWREMOTE`, `allow-remote=none`). Both now pass `--allow-remote=root`, which admits only the top-level tarball; npm 10 and 11 accept the flag as well.
...
## 2.24.0
- Add `MOPS_POCKET_IC_URL` to attach to an already-running PocketIC server instead of spawning `[toolchain] pocket-ic`. Applies to `mops build --check-deploy`, `mops test --mode replica`, `mops bench`, `mops watch`, and the tests and benchmarks `mops publish` runs. Mops still creates a PocketIC instance for the run and deletes that instance on exit (including SIGINT, bounded so a wedged server cannot hold the process hostage); it never starts or stops the remote server. A `[toolchain] pocket-ic` pin is ignored with a warning, and the binary is not downloaded. An unreachable server fails fast with an error naming the variable, and a malformed value fails every `mops` command with a clean error instead of a stack trace. Two attached-mode limitations are printed/documented: canister log output is not streamed (so `mops test --mode replica` shows no test names or `Debug.print` output), and `mops bench` records no `replicaVersion` (the remote server's version is unknown, and recording the URL would leak environment details into saved baselines and published benchmark metadata). `--replica dfx` is unchanged.
