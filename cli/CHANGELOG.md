...
- Add a fast Walrus preflight when `mops build --test-deploy` or `[build].test-deploy = true` is enabled. It analyzes the final Wasm and reports stable, actionable `MOPS-WASM-COMPLEXITY` diagnostics, including the three largest per-function contributors. Complexity from 750,000 emits an early warning, from 900,000 a critical warning, and above the IC0505 limit of 1,000,000 fails before PocketIC starts. Advisory `MOPS-WASM-SIZE` warnings report correlated IC0539 risk from total function and Walrus-local counts, but never fail or skip PocketIC. `--no-test-deploy` disables both preflight and deployment validation.
- Add `mops build --test-deploy` and `[build].test-deploy = true` to install built Wasm files on fresh PocketIC canisters and fail on deployment or initialization errors. Use `--no-test-deploy` to skip configured validation for one build. PocketIC 9.0.0 or newer, or a local PocketIC binary path, is required. Per-canister `wasmMemoryLimit` settings are applied during test deployment, and every command that resolves canisters rejects non-positive or non-integer limits. All current PocketIC error names include their canonical IC error codes. The PocketIC client is loaded only when test deployment is enabled. Canisters whose generated `.most` shows an enhanced migration chain requiring pre-existing state are skipped with a warning because they cannot be fresh-installed without a baseline Wasm.
