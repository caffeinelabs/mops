...
- **Batched registry hash fetches.** Verifying freshly downloaded packages against the registry's file hashes — an update call that goes through consensus (~2 s) — now happens in one batched `getFileHashesByPackageIds` request shared by every package verifying concurrently, instead of one request per package. On installs without a usable `mops.lock`, the request for the root dependencies starts before the downloads do, so the consensus round overlaps them; lock generation then reuses the answers for free. Installs that run from a committed lock are unchanged — they never ask the registry for hashes.
