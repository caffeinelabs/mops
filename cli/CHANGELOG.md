...
- **Breaking: the `check-stable` baseline must be a `.most` file.** `[canisters.<name>.check-stable].path` and the `mops check-stable <baseline>` argument no longer accept a `.mo` source, in `mops check` and `mops check-stable` alike; a `.mo` path fails with an error naming the fix. A `.mo` baseline described whatever that source said at the time of the run rather than what the canister actually holds, so it drifted silently and the check passed against the wrong state. Migration: `mops deployed init <canister>` creates a baseline and wires `[check-stable].path` to it, and `mops deployed` refreshes it after each deploy from the `.most` `mops build` already writes.
