...
- **Breaking**: mops no longer invokes `dfx` for anything it does itself. `dfx` does not need to be installed.
  ...
  - **Still supported, deliberately**: `mops sources` as a `dfx.json` packtool, `mops toolchain init` and the `DFX_MOC_PATH=moc-wrapper` bridge, `mops init` writing the packtool into an existing `dfx.json`, and `mops watch --deploy` / `--generate` shelling out to `dfx`. Those are for projects that *deploy* with dfx, which is their choice; mops just stops requiring dfx for its own work.
...
- `@icp-sdk/core` upgraded from `4.0.2` to `5.4.0`, which is the major the bundled `@dfinity/pic` uses. mops was held on `4.x` because `5.x` drops the IC HTTP API `v2` endpoints that the dfx replica served; with that replica gone, the two can share one copy. `dist/vendor/pic.mjs` no longer inlines a second `@icp-sdk/core` and shrinks from 1.15 MB to 600 KB. (#652)
