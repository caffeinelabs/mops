...
- Command groups say what they are for, not what they are called. `mops toolchain` reads "Install and inspect the tools mops builds with" where it used to list the tools' names in its description, and `mops deployed`, `mops user`, `mops self`, `mops owner`, `mops maintainer`, `mops migrate` and `mops docs` got the same treatment (`mops generate` already read that way). This is the one line a user sees after guessing a group name.
