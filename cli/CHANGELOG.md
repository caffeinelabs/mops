...
- Command groups say what they are for, not what they are called. `mops toolchain` reads "Install and inspect the tools mops builds with" where it used to list the tools' names in its description, and `mops user`, `mops self`, `mops owner`, `mops maintainer`, `mops migrate`, `mops generate` and `mops docs` got the same treatment. This is the one line a user sees after guessing a group name.
...
- Every command's `--help` now lists its arguments under an `Arguments:` heading with a description of each. Twenty-two commands and subcommands previously named their arguments only in the usage line, so what each one wanted had to be guessed or read in the docs: `mops search`, `mops info`, `mops add`, `mops remove`, `mops build`, `mops test`, `mops bench`, `mops format`, `mops lint`, `mops check`, `mops outdated`, `mops bump`, `mops template`, `mops deployed init`, `mops migrate new|freeze`, `mops user import`, `mops owner|maintainer add|remove` and `mops generate candid`. The four `mops toolchain` subcommands already had the section; their argument descriptions now name the tools the command accepts.
- `mops cache` is a real command group. It was a single command taking a `size|clean|show` choice, so `mops cache --help` printed `Usage: mops cache [options] <sub>` and said nothing about the three, and a bare `mops cache` errored with `missing required argument 'sub'`. The three are now subcommands, listed by both `mops cache` and `mops cache --help`. `mops cache <unknown>` now reports `unknown command '<unknown>'`, where it used to report `command-argument value '<unknown>' is invalid for argument 'sub'. Allowed choices are size, clean, show.` (both exit 1).
