...
## Next
- On moc 1.12.0+, `mops check` and `mops check-stable` check upgrade compatibility faster and report it better for canisters with `[migrations]` and a committed `.most` baseline: compatibility errors now point at your source (`src/main.mo:3.1-11.2`) instead of `(unknown location)`, and a field the initial actor requires that no migration produces now fails as an `M0267` error rather than only warning (`M0254`) — a forgotten migration that previously slipped through as a warning will now fail the check. Older moc pins, canisters without `[migrations]`, and `.mo` baselines are unaffected. Known moc issue on this path: a stable variable that fails the compatibility check is reported twice — same location, same explanation, once naming the migration and once as "the previous version". Both are the same problem and one fix clears both; mops passes moc's output through unchanged, so the duplicate goes away when moc stops emitting it.
