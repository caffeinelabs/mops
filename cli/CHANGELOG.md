...
- Add `mops build --check-deploy` and `[build].check-deploy = true` to install built Wasm files on fresh PocketIC canisters and fail on deployment or initialization errors. Use `--no-check-deploy` to skip configured validation for one build. PocketIC 9.0.0 or newer, or a local PocketIC binary path, is required. Per-canister `wasmMemoryLimit` settings are applied during deployment checks, and every command that resolves canisters rejects non-positive or non-integer limits. PocketIC errors are reported as provided by the client. Installation failures are collected across canisters before the command fails. When a failed canister's generated `.most` identifies an enhanced migration chain, Mops adds a fresh-install limitation hint without interpreting the PocketIC error text. The PocketIC client is loaded only when deployment checking is enabled.
