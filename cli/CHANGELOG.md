...
- `mops check` automatically runs stable compatibility when `[canisters.<name>.checkStable]` is configured
...
- `skipIfMissing` in `[canisters.<name>.checkStable]` silently skips when the file doesn't exist
