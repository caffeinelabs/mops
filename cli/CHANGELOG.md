...
## Next
- `mops --help` now groups commands by task — start a project, manage dependencies, publish, development, toolchain — instead of one flat alphabetical list, so the CLI is explorable without the docs site. Entries are `name, alias` with a one-line summary that fits an 80-column terminal; argument signatures and options moved to `mops <command> --help`.
