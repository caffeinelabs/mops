...
- Add a fast Walrus preflight to `mops build` that analyzes the final Wasm and reports stable, actionable `MOPS-WASM-COMPLEXITY` diagnostics, including the three largest per-function contributors. Complexity from 750,000 emits an early warning, from 900,000 a critical warning, and above the IC0505 limit of 1,000,000 fails before PocketIC starts. Advisory `MOPS-WASM-SIZE` warnings report correlated IC0539 risk from total function and Walrus-local counts, but never fail or skip PocketIC; test deployment remains authoritative for memory validation.
