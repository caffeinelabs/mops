...
- **`mops update` is single-pass.** All outdated dependencies now download in parallel through the same bounded pool `mops install` uses, `mops.toml` is rewritten once after the installs instead of once per package (so the dependency graph is re-resolved once, not once per updated package), and the registry is notified of the installs in one batched call. Registry and GitHub update lookups also run concurrently. Per-package failure handling is unchanged: a dependency that fails to update is reported, keeps its old `mops.toml` entry and exits `2`, without blocking the other updates. Each applied update is reported as `Updated <pkg> <old> -> <new>`.
