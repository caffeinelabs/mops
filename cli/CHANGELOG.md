...
## Next
- Make the CLI explorable from `--help` alone. Every command and command group now describes what it is for, every argument says what it takes and prints the usage block after a usage error, and `mops --help` groups the commands by task instead of one flat list. `cache` becomes a real command group (`size`/`show`/`clean`), so `mops cache` lists its subcommands rather than erroring, and `mops template <name>` and its `--copyright-owner` flag now work, where the argument form previously failed and opened a picker. Breaking: `mops cache --global clean` is now `mops cache clean --global`.
