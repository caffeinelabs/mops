...
// legacy (pre-v3) network file inside the installed CLI directory
// (shared between all projects and reset by npm upgrades) — read-only fallback
function getLegacyNetworkFile(): string | URL {
...
// Durable settings living in `.mops/` alongside the package cache.
// `mops cache clean` must keep these — they are user choices, not cache.
export const PROJECT_STATE_FILES = ["network"];

function getProjectNetworkFile(): string {
  let rootDir = getRootDir();
  if (!rootDir) {
    return "";
  }
  return path.join(rootDir, ".mops", "network");
}

function getGlobalNetworkFile(): string {
  return path.join(globalConfigDir, "network");
}

function readNetworkFile(file: string | URL): string {
  try {
...
// project-local `.mops/network`, then global config, then the legacy
// install-dir file (read-only, with a migration hint)
export function readSavedNetwork(): string {
  let projectFile = getProjectNetworkFile();
  if (projectFile && fs.existsSync(projectFile)) {
    let network = readNetworkFile(projectFile);
    if (network) {
      return network;
    }
  }

  let globalFile = getGlobalNetworkFile();
  if (fs.existsSync(globalFile)) {
    let network = readNetworkFile(globalFile);
    if (network) {
      return network;
    }
  }

  let legacyFile = getLegacyNetworkFile();
  if (fs.existsSync(legacyFile)) {
    let network = readNetworkFile(legacyFile);
    // 'ic' is the default — nothing to migrate
    if (network && network !== "ic") {
      process.stderr.write(
        chalk.yellow(
          `Network '${network}' was read from a legacy file inside the mops installation, shared between all projects.\n` +
            `Run ${chalk.green(`mops set-network ${network}`)} in this project (or with --global) to migrate; this fallback will be removed in a future release.\n`,
        ),
      );
      return network;
    }
...
export function setNetwork(network: string, { global = false } = {}) {
  if (global) {
    fs.mkdirSync(globalConfigDir, { recursive: true });
    fs.writeFileSync(getGlobalNetworkFile(), network + "\n");
    return;
  }
  let projectFile = getProjectNetworkFile();
  if (!projectFile) {
    console.log(
      chalk.red("Error: ") +
        `Config file 'mops.toml' not found. Run inside a mops project, or pass ${chalk.green("--global")} to set the network for all projects`,
    );
    process.exit(1);
  }
  fs.mkdirSync(path.dirname(projectFile), { recursive: true });
  fs.writeFileSync(projectFile, network + "\n");
}
