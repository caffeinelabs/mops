...
// transitive deps recurse into that same pool. The set of packages that ends
// up installed must not depend on how the pool interleaved: a dropped package
...
describe("parallel install", () => {
  ...
  test("a serial install (MOPS_CONCURRENCY=1) agrees byte for byte with a parallel one", async () => {
  ...
  test("a warm install changes nothing, and an invalid MOPS_CONCURRENCY only warns", async () => {
    const cwd = await makeTempFixture("graph");
    const cacheHome = await makeTempCacheHome();
    const env = { CI: undefined, XDG_CACHE_HOME: cacheHome };
    ...
    try {
      ...
      const warm = await cli(["install"], {
        cwd,
        env: { ...env, MOPS_CONCURRENCY: "abc" },
      });
      expect(warm.exitCode).toBe(0);
      expect(warm.stderr).toMatch(/ignoring MOPS_CONCURRENCY="abc"/);
  ...
  test("--concurrency rejects values that are not positive integers", async () => {
    const cwd = await makeTempFixture("graph");
    for (const bad of ["0", "abc", "2.5"]) {
      const result = await cli(["install", "--concurrency", bad], { cwd });
      expect(result.exitCode).not.toBe(0);
      expect(result.stderr).toMatch(/expected an integer >= 1/);
    }
  });
});
