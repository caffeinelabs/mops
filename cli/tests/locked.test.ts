...
describe("--locked", () => {
  ...
  test("plain install refuses a download that disagrees with mops.lock", async () => {
    ...
    try {
      ...
      expect(result.exitCode).toBe(1);
      expect(result.stderr).toMatch(/Hash mismatch for core@1\.0\.0/);
      // The lock answered, so the diagnosis is "one of these two is corrupt",
      // not "retry the download".
      expect(result.stderr).toMatch(/These hashes come from mops\.lock/);
