...
program
  ...
  .description(
    "Check stable variable compatibility. With no arguments, checks all canisters with [check-stable] configured. Arguments can be canister names or a baseline .most path followed by an optional canister name. The baseline is always a .most file — create one with `mops deployed init <canister>`",
  )
  .option("--verbose", "Verbose console output")
  .addOption(
    new Option(
