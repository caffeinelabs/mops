...
- **Canister IDs**: icp-cli 1.2.0 cannot declare an ID in `icp.yaml` and keeps its own untracked store, so `scripts/link-canister-ids.mjs <environment> [canister...]` points that store at `canister_ids.json` before a staging/mainnet deploy. `canister_ids.json` is the source of truth; never commit `.icp/`. The mainnet deploy scripts pass `--no-create` so a missing link fails loudly instead of creating a new canister.
