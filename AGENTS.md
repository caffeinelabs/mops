...
| `mops toolchain use <tool>` | `mops toolchain use <tool> <version>` (e.g. `pocket-ic 14.0.0`). `latest` works but may resolve to a version incompatible with the shipped client. |
