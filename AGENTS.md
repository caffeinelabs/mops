...
- **Keep docs in sync.** CLI command docs live in `docs/docs/cli/` and config reference in `docs/docs/09-mops.toml.md`. The same feature often appears in both — update all relevant pages. `docs/docs/` is the in-development line served at the site root; `docs/versioned_docs/version-2.x/` is a frozen snapshot — back-port only correctness fixes for behavior that also exists in 2.x.
