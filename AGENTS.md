...
- **Asset canisters are mid-migration.** `docs` runs dfinity's current certified-assets canister (`@dfinity/static-site`); `assets`, `blog`, `play-frontend` and `cli` still run the legacy dfx-era one (`@dfinity/asset-canister`, which installs dfinity/sdk `assetstorage`). They behave differently and the difference is load-bearing:
  - **Unknown paths.** Legacy answers them with the root `index.html` at **HTTP 200**, so a status check proves nothing — that is why every deploy assertion in `release.yml` and `.github/actions/deploy-canister/` greps the body. certified-assets returns a real **404** and uses a root `404.html` when the build ships one.
  - **Dot-directories.** Legacy uploads `.well-known/` only because an `.ic-assets.json` opts it in; drop that file and the custom domain stops resolving. certified-assets uploads `.well-known/` by default and needs no such file — `docs/static/.ic-assets.json` is gone for that reason.
  - **Clean URLs.** certified-assets canonicalises them, so `docs/docusaurus.config.js` must keep `trailingSlash: false`; see the comment there. Legacy does no canonicalising.
  - **No default headers.** certified-assets sets no `Cache-Control` and no security headers — a `_headers` file at the asset root is the only way to add them. Legacy set none either, so the migration changed nothing here, but it is the place to add caching for content-hashed `/assets/*`.
  - **Cutover is `-m reinstall`.** The two are different wasms with incompatible state, so `icp deploy` in its default `auto` mode picks an upgrade that cannot work. Migrating a canister is a one-off `icp deploy <name> -e ic --identity mops --no-create -m reinstall`, which wipes the assets and re-syncs them; the canister ID and therefore the custom-domain registration survive, so the only cost is downtime for the length of the sync.
  - **`static-site` is 0.x.** Per its own policy a patch bump upgrades in place and a **minor bump is breaking** — it reinstalls and re-uploads everything. Read the release notes before moving the pin.
  - **`cli` cannot move yet**: `static-site` takes a single `dir` and that canister serves two (`cli-releases/` and `cli-releases/frontend/dist/`).
