...
- **The root `mops.lock` is committed; fixture lockfiles are not.** Keep it in sync (`mops install`) whenever the root `mops.toml` changes — the `sites` job installs with `--locked` and fails on a stale one. Fixtures under `cli/tests/` and `test/` have no committed lock on purpose, because the suite deletes, rewrites and corrupts them, so `mops.lock created. Commit this file.` in a CI log is expected there rather than a sign of staleness.
