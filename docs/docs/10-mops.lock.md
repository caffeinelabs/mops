...
A lockfile generated by a CLI older than 3.0 has no integrity record for `repo = "..."` dependencies, so `--locked` fails on it until a plain `mops install` regenerates it. Projects with no GitHub dependencies are unaffected.
