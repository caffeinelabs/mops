...
Use `mops build --check-deploy`, or set `[build].check-deploy = true` for every build, to install each built Wasm on a fresh PocketIC canister and catch module validation, initialization, and installation failures. Pin pocket-ic 9.0.0 or newer, or a local PocketIC binary path, in `[toolchain]` first. Use `--no-check-deploy` to skip configured validation once. The command uses each canister's `initArg`, or `()` when omitted. Set `wasmMemoryLimit` to a positive integer byte limit on a canister to check deployment under that limit. PocketIC errors are reported as provided by the client, and installation failures are collected across canisters. If the generated `.most` identifies an enhanced migration chain, a failed fresh installation includes a hint that converted legacy migrations may require baseline state. The failure is preserved; validate the upgrade against representative state or disable `check-deploy` for projects that cannot support fresh installation.
